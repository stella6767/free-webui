...
//        attributes["hx-vals"] = """js:{"chatId": ${chat?.id ?: 0}}"""
        ...
        div("absolute bottom-0 left-0 w-full flex p-5") {
            id = "bottom-logo-box"
            a {
                href = "https://github.com/stella6767/free-webui"
                target = "_blank"
                svg("cursor-pointer") {
                    attributes["width"] = "50"
                    attributes["height"] = "50"
                    attributes["viewbox"] = "0 0 98 96"
                    path {
...
fun DIV.chatsNavView(chats: Page<ChatRespDto>, currentChat: ChatRespDto?) {
    nav {
        classes = setOf("mt-2")
        if (!chats.isLast) {
            attributes["hx-get"] = "/chats?page=${chats.pageable.pageNumber + 1}"
...
private fun DIV.dropDownMenuView(chat: ChatRespDto) {
...
fun DIV.setModelFormView(setting: OllamaResponseRto) {
    ...
    form {
        ...
        attributes["hx-on--after-swap"] = "closeModal()"
        div {
